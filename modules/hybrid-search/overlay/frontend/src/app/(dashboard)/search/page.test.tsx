...
vi.mock('@/lib/modules/hooks', () => ({useModules:vi.fn(()=>({data:[]}))}))
vi.mock('@/components/search/HybridStatus', () => ({HybridStatus:()=>null,RetrievalFeedback:()=>null}))
...
describe('SearchPage', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    vi.mocked(useModules).mockReturnValue({data:[]} as unknown as ReturnType<typeof useModules>)
  ...
  it('defaults to hybrid when the module is enabled', () => {
    vi.mocked(useModules).mockReturnValue({data:[{id:'hybrid-search',active:true}]} as unknown as ReturnType<typeof useModules>)
    render(<SearchPage />)
    expect(screen.getByRole('radio', {name:/Hibrit arama|Hybrid search|modules.hybrid.title/})).toHaveAttribute('data-state','checked')
  })

...
export default function SearchPage() {
  ...
  const hybridEnabled = modules?.some(m => m.id === 'hybrid-search' && m.active) ?? false
  ...
  const handleSearch = useCallback(() => {
    ...
    searchMutation.mutate({
      query: searchQuery,
      type: searchType === 'hybrid' && !hybridEnabled ? 'text' : searchType,
      limit: 30,
  ...
  }, [searchQuery, searchType, searchSources, searchNotes, scopeNotebookIds, searchMutation, hybridEnabled])
  ...
  useEffect(() => {
    ...
    if (hasAutoTriggeredRef.current || !urlQuery || !modules) return
  ...
  }, [urlQuery, urlMode, modelsLoading, modelDefaults, handleSearch, handleAsk, modules])
  ...
  return (
    <AppShell>
      <div className="flex-1 overflow-y-auto p-4 md:p-6">
        ...
        {hybridEnabled && <HybridStatus/>}
        {ask.retrieval.map((value, i)=><RetrievalFeedback key={i} value={value}/>)}
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  {searchType === 'hybrid' && hybridEnabled ? <p className="text-xs text-muted-foreground">{t('modules.hybrid.description')}</p> : searchType === 'vector' ? (
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2" role="group" aria-labelledby="search-type-label">
                    ...
                    <RadioGroup
                      name="search-type"
                      value={searchType === 'hybrid' && !hybridEnabled ? 'text' : searchType}
                      onValueChange={(value: 'text' | 'vector' | 'hybrid') => setSearchType(value)}
                      disabled={modelsLoading || searchMutation.isPending}
                    >
                      {hybridEnabled && <div className="flex items-center space-x-2"><RadioGroupItem value="hybrid" id="hybrid"/><Label htmlFor="hybrid">{t('modules.hybrid.title')}</Label><Badge variant="secondary">{t('modules.hybrid.recommended')}</Badge></div>}
                ...
                {searchMutation.data && (
                  <div className="mt-6 space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      <Badge variant="outline">{searchMutation.data.search_type === 'hybrid' ? t('modules.hybrid.title') : searchMutation.data.search_type === 'text' ? t('searchPage.textSearch') : t('searchPage.vectorSearch')}</Badge>
                    </div>

                    {searchMutation.data.diagnostics && <RetrievalFeedback value={searchMutation.data.diagnostics}/>}
                            ...
                            <CardContent className="pt-4">
                              <div className="flex items-start justify-between gap-4">
                                <div className="flex-1">
                                  ...
                                  <Badge variant="secondary" className="ml-2 font-mono text-[11px]" title={t('modules.hybrid.scoreHelp')}>
                                    {result.final_score.toFixed(2)}
                                  </Badge>
                                </div>
                              </div>

                              {result.matches?.[0] && <p className="mt-3 line-clamp-3 whitespace-pre-wrap text-sm leading-relaxed text-muted-foreground">{result.matches[0]}</p>}

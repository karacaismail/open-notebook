  ...
  return useMutation({
    mutationFn: async (params: SearchRequest) => {
      ...
      const processedResults = response.results.map(result => ({
        ...result,
        final_score: result.final_score ?? result.relevance ?? result.similarity ?? result.score ?? 0
      }))

      // Sort by final_score descending
      if (response.search_type !== 'hybrid') processedResults.sort((a, b) => b.final_score - a.final_score)

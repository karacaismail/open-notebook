...
async def search_index_status():
    return await SearchAdapter.engine().status()
...
async def search_knowledge_base(search_request: SearchRequest):
    ...
    try:
        ...
        if search_request.type == "hybrid":
            results, diagnostics = await SearchAdapter.engine().search(
                search_request.query, search_request.limit, search_request.search_sources,
                search_request.search_notes, notebook_ids)
    ...
    try:
        ...
        async for chunk in ask_graph.astream(  # type: ignore[call-overload]
            ...
            if "agent" in chunk:
                ...
                for diagnostic in chunk["provide_answer"].get("retrieval", []):
                    yield "data: " + json.dumps({"type": "retrieval", **diagnostic}) + "\n\n"
...
async def ask_knowledge_base(ask_request: AskRequest):
    ...
    try:
        ...
        if not SearchAdapter.enabled() and not await model_manager.get_embedding_model():
            raise HTTPException(
                status_code=400,

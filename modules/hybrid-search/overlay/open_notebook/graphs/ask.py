...
class ThreadState(TypedDict):
    ...
    notebook_ids: list
    retrieval: Annotated[list, operator.add]
...
async def call_model_with_messages(state: ThreadState, config: RunnableConfig) -> dict:
    try:
        ...
        strategy.searches = list({s.term.strip(): s for s in strategy.searches if s.term.strip()}.values())[:5]
        if SearchAdapter.enabled():
            # Preserve identifiers in the user query even when the planner paraphrases.
            original = state["question"].strip()[:4000]
            if all(s.term.strip() != original for s in strategy.searches):
                strategy.searches = [Search(term=original, instructions="Find direct evidence for the original question, preserving exact identifiers."), *strategy.searches[:4]]
...
async def provide_answer(state: SubGraphState, config: RunnableConfig) -> dict:
    try:
        ...
        diagnostic = None
        if SearchAdapter.enabled():
            engine = SearchAdapter.engine()
            from open_notebook.modules.hybrid_search.service import settings
            results, diagnostic = await engine.search(state['term'], settings()['ask_results'], True, True, state.get('notebook_ids') or None)
        else:
            results = await vector_search(state['term'], 10, True, True, notebook_ids=state.get('notebook_ids') or None)
        if not results:

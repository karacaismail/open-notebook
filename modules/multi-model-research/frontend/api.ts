...
export type ResearchStage = { account_profile?:'preliminary_research'|'preliminary_merge'; depends_on?:string[]; policy?:ResearchPolicy; id:string; provider:string; round:number; mode:'import'|'account'|'browser'; status:string; attempts:number; report:ResearchReport|null; error:string|null; usage:Record<string,number>|null; note_id:string|null; browser_progress:BrowserProgress|null; retry_index:number; next_retry_at:string|null }
export type ExecutionMode = 'imports'|'browser'
export type ResearchRun = { id:string; question:string; scope:string; as_of:string; language:string; auto_synthesize:boolean; execution_mode:ExecutionMode; preliminary?:boolean; control_state?:'stopping'|'stopped'|'cancelled'|'stop_failed'|null; control_error?:string|null; paused:boolean; status:string; stages:ResearchStage[]; created_at:string; updated_at:string; notebook_id:string|null; sync_error:string|null }
export type ControlSnapshot = {status:string;paused:boolean;control_state:string|null;stages:(string|number)[][]}
export function controlSnapshot(run: ResearchRun): ControlSnapshot { return {status:run.status,paused:run.paused,control_state:run.control_state || null,stages:run.stages.map(s=>[s.id,s.status,s.attempts])} }
...
export type ResearchCreate = {preliminary?:boolean;question:string;scope:string;as_of:string;language:string;auto_synthesize:boolean;execution_mode:ExecutionMode}
...
export type ResearchPacket = Partial<InputBudget> & {prompt:string;sha256:string;estimated_tokens:number;automatic_input_limit:number;report_count:number;policy?:ResearchPolicy;evidence_packet?:{sha256:string;bytes:number;format:string}}

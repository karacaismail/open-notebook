...
export type InputBudget = {raw_tokens:number;transport_raw_tokens:number;counted_tokens:number;automatic_input_limit:number;effective_raw_limit:number;remaining_input_tokens:number;utilization:number;fits:boolean;tokenizer:string;token_margin:{multiplier:number;overhead_tokens:number;basis:string}}
...
export type ResearchPacket = Partial<InputBudget> & {prompt:string;sha256:string;estimated_tokens:number;automatic_input_limit:number;report_count:number}

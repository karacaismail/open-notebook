...
export function ResearchActionDialog({ run, action, stageId, onClose, onConfirm }: {
  ...
  const affected = run.stages.filter(s => s.status !== 'completed' && (!stageId || s.id === stageId) && (stageId || restarting || s.status === 'running'))

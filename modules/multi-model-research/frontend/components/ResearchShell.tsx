...
/** Responsive shell scoped to research; other application pages are unchanged. */
  ...
  return <div className="flex h-dvh overflow-hidden">
    ...
    <main className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
      <div className="flex items-center gap-2 border-b bg-card px-3 py-2 md:hidden">
        <Dialog><DialogTrigger asChild><Button variant="ghost" size="icon" className="size-11" aria-label={t('research.navigation')}><Menu aria-hidden className="size-5" /></Button></DialogTrigger>
          <DialogContent className="start-0 top-0 h-dvh w-72 max-w-72 translate-x-0 translate-y-0 rounded-none border-y-0 border-s-0 p-0 sm:max-w-72">
            <DialogTitle className="sr-only">{t('research.navigation')}</DialogTitle>

...
def plan_for(engine, run, stage, prompt):
    ...
    if not all(engine.measure_input(prefix+result_instructions(['P1'],final=True)+pin,peer)['utilization'] < .8 for peer,prefix in prefixes):
        raise PreparationError('The protected claim register requires a larger verified budget; no claims were removed.')
...
def summary(plan):
    return {'plan_sha256':digest(json.dumps(plan,ensure_ascii=False,sort_keys=True)),
...
async def execute(engine, run, stage, prompt):
    ...
    def validated_report(response,ids):
    ...
    async def call(key, data, ids, final=False):
        request=prefix+result_instructions(ids,final=final)+data
        ...
        for depth in range(4):
            ...
            def data(group):
                return envelope(json.dumps({'source_sha256':plan['source_sha256'],
                    ...
                    'evidence_register':plan.get('protected_register',{}),
                    'source_inventory':plan.get('source_inventory',[]),'findings':group},ensure_ascii=False,separators=(',',':')))
            merged=data(nodes)
            if engine.measure_input(prefix+result_instructions(ids,final=stage['round']==4)+merged,stage)['utilization']<=.95:
                result=await call('final-'+str(depth),merged,ids,final=stage['round']==4)
                async with engine.lock:
                    latest=await engine.get(run['id']);current=engine.stage(latest,stage['id'])
                    current['preparation']=dict(summary(plan),status='completed',completed_calls=len(state['jobs']))
                    await engine.store.save(latest)
                return result,{'segmented':True,'calls':len(usages),'measurements':usages,
                               'coverage':ids,'source_sha256':plan['source_sha256']}
            groups=[];group=[]
            for node in nodes:
                trial=group+[node];covered=[i for n in trial for i in n['ids']]
                if group and engine.measure_input(prefix+result_instructions(covered)+data(trial),stage)['utilization']>.7:
            ...
            for index,group in enumerate(groups):
                ...
                result=await call(f'reduce-{depth}-{index}',data(group),ids)
                next_nodes.append({'ids':ids,'report':result})

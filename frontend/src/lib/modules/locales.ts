...
const en = {
  ...
  fields: {
"searchCandidates": "Candidates per retriever", "searchCandidatesHelp": "More candidates can improve recall with additional work.", "rerankLimit": "Passages to rerank", "rerankLimitHelp": "Bound local neural inference work per search.", "askResults": "Documents per Ask query", "askResultsHelp": "Distinct documents for the answer; up to three passages each.", "rerankEnabled": "Local neural reranking", "rerankEnabledHelp": "Falls back to rank fusion with feedback if unavailable.", "searchSync": "Index refresh interval (seconds)", "searchSyncHelp": "Changed documents are reindexed in the background. Original data is retained.", "weakRelevance": "Weak match threshold (per mille)", "weakRelevanceHelp": "When the best reranker score falls below this, neural ordering is set aside, the fused candidates are kept and the result is marked as a weak match. 1 per mille only catches complete misses; raise it to be warned about thin matches. This is a local heuristic, not a probability of being correct.", "metadataCache": "Document list reuse (seconds)", "metadataCacheHelp": "Searches reuse the document list instead of rehashing the whole archive each time. A stale entry only marks a document pending and triggers a resync. Zero disables reuse.",
